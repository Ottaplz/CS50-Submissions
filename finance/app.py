...
def index():
    ...
    for row in range(len(symbols)):
        ...
        sales = 0
        owned = 0
        purchases_raw = db.execute(
            "SELECT AMOUNT FROM stocks WHERE user_id=? AND type=? AND symbol=?", sid, "purchase", symbols[row]["symbol"])
        for row2 in range(len(purchases_raw)):
            purchases += purchases_raw[row2]["AMOUNT"]
        sales_raw = db.execute(
            "SELECT AMOUNT FROM stocks WHERE user_id=? AND type=? AND symbol=?", sid, "sale", symbols[row]["symbol"])
...
def buy():
    ...
    # User reached route via POST (as by submitting a form via POST)
        ...
        db.execute("INSERT INTO stocks (user_id, symbol, price, amount, date, type) VALUES(?, ?, ?, ?, ?, ?)",
                   sid, symbol, value, amount, todays_date, "purchase")
...
def sell():
    ...
    # User reached route via POST (as by submitting a form via POST)
        ...
        sid = session["user_id"]
        bought_raw = db.execute(
            "SELECT amount FROM stocks WHERE user_id=? AND symbol=? AND type=?", sid, symbol, "purchase")
        sold_raw = db.execute(
            "SELECT amount FROM stocks WHERE user_id=? AND symbol=? AND type=?", sid, symbol, "sale")
        bought = 0
        sold = 0
        ...
        todays_date = datetime.datetime.now()
        db.execute("INSERT INTO stocks (user_id, symbol, price, amount, date, type) VALUES(?, ?, ?, ?, ?, ?)",
                   sid, symbol, value, amount, todays_date, "sale")
...
def addfunds():
    ...
    # User reached route via POST (as by submitting a form via POST)
        ...
        # Recieve input from user
        funds = request.form.get("addfunds")
        # Ensure a valid number of funds
        if not funds:
            return apology("Please enter a valid amount of funds to add")
        if funds.isdigit() == False:
            return apology("Please enter a valid amount of funds")
        amount = int(funds)
        if amount < 1:
            return apology("Please enter a valid amount of funds")
        # Add new funds to existing user funds
        sid = session["user_id"]
        val = db.execute("SELECT cash FROM users WHERE id==?", sid)
        newfunds = val[0]["cash"] + int(funds)
        db.execute("UPDATE users SET cash=? WHERE id=?", newfunds, sid)
        return redirect("/")
